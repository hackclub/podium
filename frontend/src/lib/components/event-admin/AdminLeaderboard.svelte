<script lang="ts">
  ...
  import { validateDaydreamURLs, isDaydreamEvent } from '$lib/daydreamValidation';
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each projects as project, index}
          <div class="relative">
            ...
            {#if isDaydream}
              {@const validation = validateDaydreamURLs(project.demo, project.repo)}
              <div class="absolute top-2 -right-4 z-20">
                <div class="bg-base-100 shadow-lg border border-base-300 rounded-lg p-3 text-sm max-w-56">
                  ...
                  <div class="text-sm font-medium {validation.isValid ? 'text-success' : 'text-red-600'} break-words">
                    {validation.message}
                  </div>
