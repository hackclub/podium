...
export const itchioRegex = /^(https?:\/\/)?[a-zA-Z0-9\-_]+\.itch\.io\/[a-zA-Z0-9\-_]+/;
export const githubRegex = /^(https?:\/\/)?(github\.com|gitee\.com)\/[a-zA-Z0-9\-_]+\/[a-zA-Z0-9\-_.]+/;
 ...
 */
export function validateDaydreamURLs(demoUrl: string | null | undefined, repoUrl: string | null | undefined): URLValidationResult {
  ...
  const issues = [];
  if (demo && !demoValid) {
    issues.push("demo URL must be a valid itch.io URL (format: https://username.itch.io/gamename)");
  }
  if (repo && !repoValid) {
    issues.push("repo URL must be a valid GitHub/Gitee URL (format: https://github.com/username/repository)");
  }
 ...
 */
export function isDaydreamEvent(event: { feature_flags_list?: string[] }): boolean {
