...
def magic_url(temp_user_tokens: Mapping[str, Any]) -> str:
    ...
    return (
        f"Use this magic login link to sign in: {temp_user_tokens['magic_link_url']}."
    )
...
def create_temp_user_tokens(app_public_url: str) -> Dict[str, str]:
    ...
    if db is None or auth_router is None:
        raise ImportError(
            "Required modules (podium.db, podium.routers.auth) not available"
        )
...
def verify_event_exists(event_id: str, expected_name: str, expected_owner: str) -> bool:
    ...
    try:
        ...
        return event["fields"]["name"] == expected_name and expected_owner in event[
            "fields"
        ].get("owner", [])
    except Exception:
        return False
...
def verify_project_exists(
    project_id: str, expected_event_id: str, expected_owner: str
) -> bool:
    ...
    try:
        ...
        return expected_event_id in project["fields"].get(
            "event", []
        ) and expected_owner in project["fields"].get("owner", [])
    except Exception:
        return False


def verify_vote_exists(
    vote_id: str, expected_event_id: str, expected_project_id: str, expected_voter: str
) -> bool:
    ...
    try:
        ...
        return (
            expected_event_id in vote["fields"].get("event", [])
            and expected_project_id in vote["fields"].get("project", [])
            and expected_voter in vote["fields"].get("voter", [])
        )
    except Exception:
        return False
...
def assert_event_created(
    event_name: str, expected_owner: str, expected_description: str = None
) -> str:
    ...
    assert event["fields"]["name"] == event_name, (
        f"Event name mismatch: expected '{event_name}', got '{event['fields']['name']}'"
    )
    assert expected_owner in event["fields"].get("owner", []), (
        f"User {expected_owner} is not the owner of event"
    )

    if expected_description is not None:
        assert event["fields"]["description"] == expected_description, (
            f"Event description mismatch: expected '{expected_description}', got '{event['fields']['description']}'"
        )
...
def assert_user_joined_event(event_id: str, user_id: str) -> None:
    ...
    assert verify_user_in_event_attendees(event_id, user_id), (
        f"User {user_id} was not added as attendee to event {event_id}"
    )
