...
def format_record_id(
    record_id: str, base_id: str, table_id: str, label: str = None
) -> str:
...
def get_daydream_attendees() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        print(
            f"Found {len(records)} Daydream attendees from view {DAYDREAM_ATTENDEES_VIEW_ID}"
        )
...
def create_display_name(
    first_name: str, last_name: str, preferred_name: str = ""
) -> str:
...
def create_or_find_user(daydream_event: Dict[str, Any]) -> Optional[str]:
    ...
    user_data = UserSignupPayload(
        ...
        display_name=fields.get("poc_preferred_name", "").strip()
        or fields.get("poc_first_name", "").strip(),
        phone=get_phone_value(
            fields.get("poc_phone", "")
        ),  # Use phone as-is if available
    ...
    try:
        ...
        record = users_table.create(user_data.model_dump())
        user_id = record["id"]
        user_link = format_record_id(
            user_id, PRODUCTION_BASE_ID, PROD_USERS_TABLE_ID, "View User"
        )
...
def create_daydream_event(
    daydream_event: Dict[str, Any], owner_id: str
) -> Optional[str]:
    ...
    if duplicate_check["exists"]:
        if duplicate_check["by_name"] and duplicate_check["by_slug"]:
            ...
            name_link = format_record_id(
                ...
                PRODUCTION_BASE_ID,
                PROD_EVENTS_TABLE_ID,
                "Name Match",
            )
            slug_link = format_record_id(
                duplicate_check["slug_match"]["id"],
                PRODUCTION_BASE_ID,
                PROD_EVENTS_TABLE_ID,
                "Slug Match",
            )
    ...
    if TEST_RUN:
        print(
            f"[DRY RUN] Would create event: {temp_event.model_dump(exclude={'id', 'max_votes_per_user', 'owned', 'feature_flags_list'})}"
        )
    ...
    try:
        ...
        event_id = record["id"]
        event_link = format_record_id(
            event_id, PRODUCTION_BASE_ID, PROD_EVENTS_TABLE_ID, "View Event"
        )
...
def migrate_attendees_for_event(
    daydream_event_id: str,
    podium_event_id: str,
    daydream_attendees: List[Dict[str, Any]],
) -> Dict[str, Any]:
    ...
    for attendee in event_attendees:
        ...
        attendee_name = (
            attendee.get("fields", {}).get("first_name", "")
            + " "
            + attendee.get("fields", {}).get("last_name", "")
        )
        ...
        if user_id == "DUPLICATE_EMAIL":
            print(
                f"      ⚠️  {attendee_name} ({attendee_email}) - Skipped (duplicate email)"
            )
            ...
            print(
                f"      ❌ {attendee_name} ({attendee_email}) - Failed to create/find user"
            )
        ...
        if not TEST_RUN:
            try:
                ...
                if user_id not in current_attendees:
                    ...
                    events_table.update(
                        podium_event_id, {"attendees": current_attendees}
                    )
        ...
        attendee_link = airtable_link(
            attendee_id, DAYDREAM_BASE_ID, DAYDREAM_ATTENDEES_TABLE_ID
        )
        ...
        successful_attendees.append(
            {
                ...
                "user_link": user_link,
                "podium_user_record_id": user_id
                if not user_id.startswith("dry_run_")
                else None,
    ...
    print(
        f"   📊 Attendees: {len(successful_attendees)} successful, {len(failed_attendees)} failed, {len(duplicate_attendees)} duplicates"
    )
...
def save_migration_progress(results: Dict[str, Any], results_file: str) -> None:
...
def verify_record_exists(record_id: str, base_id: str, table_id: str) -> bool:
    ...
    try:
        ...
        if "403" in str(e) or "INVALID_PERMISSIONS" in str(e):
            print(
                f"   ⚠️  Permission error verifying record {record_id} - assuming exists"
            )
            return True
...
def verify_migration_records(progress: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for migration in progress.get("successful_migrations", []):
        ...
        # Verify event exists
        event_exists = verify_record_exists(
            podium_event_id, PRODUCTION_BASE_ID, PROD_EVENTS_TABLE_ID
        )
        # Verify owner exists
        owner_exists = verify_record_exists(
            owner_id, PRODUCTION_BASE_ID, PROD_USERS_TABLE_ID
        )

        if event_exists and owner_exists:
            verified_progress["successful_migrations"].append(migration)
            print(
                f"   ✅ {migration.get('event_name', 'Unknown')} - Event and owner verified"
            )
        else:
            ...
            print(
                f"   ❌ {migration.get('event_name', 'Unknown')} - Verification failed"
            )
    ...
    verified_progress["skipped_migrations"].extend(
        progress.get("skipped_migrations", [])
    )
    verified_progress["duplicate_migrations"].extend(
        progress.get("duplicate_migrations", [])
    )
    ...
    print(
        f"   📊 Verification complete: {verified_progress['successful']} verified, {verified_progress['failed']} failed verification"
    )
...
def migrate_daydream_events(
    resume_from_file: str = None, skip_verification: bool = False
):
    ...
    print(
        f"Per-event confirmation: {'ENABLED' if ENABLE_PER_EVENT_CONFIRMATION else 'DISABLED'}"
    )
    if not TEST_RUN:
    ...
    if resume_from_file:
        ...
        if previous_progress:
            print(
                f"   ✅ Loaded previous progress: {previous_progress.get('total_processed', 0)} events processed"
            )
        ...
        if latest_file:
            ...
            if response in ["y", "yes"]:
                ...
                if previous_progress:
                    print(
                        f"   ✅ Resuming from: {previous_progress.get('total_processed', 0)} events processed"
                    )
                    # Verify records still exist (unless skipped)
                    if not skip_verification:
                        previous_progress = verify_migration_records(previous_progress)
    ...
    if previous_progress:
        ...
        events_to_migrate = [
            event for event in events_to_migrate if event.get("id") not in processed_ids
        ]
    ...
    if previous_progress and resume_from_file:
        ...
        print(
            f"📊 Previous progress: {len(successful_migrations)} successful, {len(failed_migrations)} failed, {len(skipped_migrations)} skipped, {len(duplicate_migrations)} duplicates"
        )
    else:
        successful_migrations = []
        failed_migrations = []
    ...
    for i, event in enumerate(events_to_migrate, 1):
        ...
        attendee_results = migrate_attendees_for_event(
            event_id, podium_event_id, daydream_attendees
        )
        ...
        successful_migrations.append(
            {
                ...
                "event_name": event_name,
                "owner_id": owner_id,
                "daydream_event_link": airtable_link(
                    event_id, DAYDREAM_BASE_ID, DAYDREAM_TABLE_ID
                ),
                "podium_event_link": airtable_link(
                    podium_event_id, PRODUCTION_BASE_ID, PROD_EVENTS_TABLE_ID
                ),
                "podium_user_link": airtable_link(
                    owner_id, PRODUCTION_BASE_ID, PROD_USERS_TABLE_ID
                ),
    ...
    if successful_migrations:
        ...
        for migration in successful_migrations:
            ...
            print(
                f"     Attendees: {attendee_data.get('successful', 0)}/{attendee_data.get('total_attendees', 0)} migrated"
            )
    ...
    try:
        ...
        while i < len(sys.argv):
            ...
            if arg == "--resume" and i + 1 < len(sys.argv):
                ...
                print(
                    "  --resume <filename>      Resume migration from a specific results file"
                )
                print(
                    "  --skip-verification     Skip record verification when resuming"
                )
        ...
        migrate_daydream_events(
            resume_from_file=resume_file, skip_verification=skip_verification
        )
